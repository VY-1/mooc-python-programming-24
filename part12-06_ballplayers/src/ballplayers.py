...
def most_goals(ball_players: list):
    return max(ball_players, key=lambda player: player.goals).name

def most_points(ball_players: list):
    player_most_points =  max(ball_players, key=lambda player: player.goals + player.passes)

    return player_most_points.name, player_most_points.number
